...
class AuthController extends Controller
{
    ...
    public function sendDeletionCode(Request $request): JsonResponse
    {
        ...
        if (Cache::has($cooldownKey)) {
            $secondsLeft = max(1, Cache::get($cooldownKey) - time());
        ...
        Cache::put("user_del_code_{$user->id}", $code, now()->addMinutes(10));
    ...
    public function deleteAccount(Request $request): JsonResponse
    {
        ...
        if ($request->filled('verification_code')) {
            $cachedCode = Cache::get("user_del_code_{$user->id}");
            ...
            Cache::forget("user_del_code_{$user->id}");
        }
